...
namespace ShellLinkPlus
{
    ...
    public class ShellLink : IDisposable
    {
        ...
        // PropID:    26
        // Type:      Guid -- VT_CLSID
        private readonly PropertyKey PKEY_AppUserModel_ToastActivatorCLSID =
        ...
        public string ToastActivatorCLSID
        {
            get
            {
                using (PropVariant pv = new PropVariant())
                {
                    VerifySucceeded(PropertyStore.GetValue(PKEY_AppUserModel_ToastActivatorCLSID, pv));
            ...
            set
            {
                using (PropVariant pv = new PropVariant(value))
                {
                    pv.VarType = VarEnum.VT_CLSID;
                    VerifySucceeded(PropertyStore.SetValue(PKEY_AppUserModel_ToastActivatorCLSID, pv));
                    VerifySucceeded(PropertyStore.Commit());
                }
            }
        }
